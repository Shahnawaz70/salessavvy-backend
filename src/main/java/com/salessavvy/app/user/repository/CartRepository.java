...
public interface CartRepository extends JpaRepository<Cart_Items, Integer> {
	...
	@Query
	("SELECT c FROM Cart_Items c JOIN FETCH c.product p LEFT JOIN FETCH ProductImage pi ON p.productId = pi.product.productId WHERE c.user.userId=:userId")
	...
	// count the total quantity of the cart
